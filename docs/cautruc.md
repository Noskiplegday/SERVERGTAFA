...
| `logs/runtime.log`, `logs/` | Log runtime, dùng để debug, không chứa logic gameplay. |
...
| File | Chứa gì |
|---|---|
| `Core/Database.pwn` | Cau hinh Supabase PostgreSQL, handle database, helper query chung, log loi SQL. |
| `Core/Account.pwn` | Welcome dialog, đăng ký nhiều bước, đăng nhập username/password, hash/check mật khẩu, email account. |
| `Core/Player_Data.pwn` | Load/save dữ liệu player: tiền, skin, level, exp, job, vị trí. |
| `Core/Session.pwn` | Trạng thái đăng nhập runtime trên server game, chặn chơi khi chưa login, timer auto-save/paycheck, cleanup khi disconnect. |
...
6. Kiem tra `logs/runtime.log`, `logs/errors.log`, `logs/warnings.log`.
7. Test trong game.
...
### Luồng Account/Session

- Khi player connect: `OnPlayerConnect` gọi `Account_OnPlayerConnect`, reset session runtime và hiển thị welcome dialog.
- Đăng ký: welcome -> username -> password -> verify password -> email -> bcrypt hash -> insert `users`.
- Đăng nhập: welcome -> username -> load user -> password -> bcrypt check -> `Session_Login`.
- Username/email input được trim trước khi query; lookup account dùng so sánh case-insensitive.
- Session hiện lưu trong RAM của gamemode (`PlayerData[playerid][pLoggedIn]`, timer id, data cache). open.mp chỉ quản lý connection/playerid; logic login là của gamemode.
- Player chưa login bị giữ trong spectate mode và `OnPlayerRequestClass`/`OnPlayerRequestSpawn` trả về 0 để chặn spawn.
- Khi disconnect: `OnPlayerDisconnect` lưu `PlayerData_Save`, dừng job/call runtime, kill timer trong `Session_Cleanup`, rồi reset slot player.
- `users` là schema Core hiện tại: `id`, `username`, `password_hash`, `email`, `money`, `bank_money`, `skin`, `level`, `exp`, `job_id`, `admin_level`, `pos_x`, `pos_y`, `pos_z`, `angle`, `last_login_at`, `created_at`, `updated_at`.
